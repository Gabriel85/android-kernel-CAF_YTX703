...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		...
		if (is_in_donotkill_proc_list(p->comm)) {
			...
			lowmem_print(2, "The process '%s' is inside the donotkill_proc_names\n", p->comm);
			...
			lowmem_print(2, "The process '%s' is inside the donotkill_sysproc_names\n", p->comm);
		}
		#endif
	 ...
	 * - if there is no killable process, then check if a managed user process has been found,
	 *   if so kill it to prevent system slowdowns, hangs, etc.
	 * - do NEVER kill managed system processes */
	for (proc_type = KILLABLE_PROCESS; proc_type < (MANAGED_PROCESS_TYPES-1); proc_type++) {
		if (selected[proc_type]) {
			long cache_size = other_file * (long)(PAGE_SIZE / 1024);
			long cache_limit = minfree * (long)(PAGE_SIZE / 1024);
			long free = other_free * (long)(PAGE_SIZE / 1024);
			trace_lowmemory_kill(selected[proc_type], cache_size, cache_limit, free);
